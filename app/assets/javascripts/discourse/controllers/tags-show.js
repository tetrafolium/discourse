...
export default Controller.extend(BulkTopicSelection, FilterModeMixin, {
  application: controller(),
    ...
    tag: null, additionalTags: null, list: null,
    canAdminTag: alias("currentUser.staff"), navMode: "latest", loading: false,
    canCreateTopic: false, order: "default", ascending: false, status: null,
    state: null, search: null, max_posts: null, q: null, showInfo: false,

    categories: alias("site.categoriesList"),

    @discourseComputed("list", "list.draft") createTopicLabel(list, listDraft) {
    return listDraft ? "topic.open_draft" : "topic.create";
  }
  ,

    @discourseComputed("canCreateTopic", "category", "canCreateTopicOnCategory",
                       "tag", "canCreateTopicOnTag")
    createTopicDisabled(canCreateTopic, category, canCreateTopicOnCategory, tag,
                        canCreateTopicOnTag) {
  ...
  ,
    ...
    return NavItem.buildList(category, { tagId, filterType, noSubcategories });
  }
  ,

    @discourseComputed("category") showTagFilter() {
  ...
  ,
    ...
    @observes("list.canLoadMore") _showFooter() {
