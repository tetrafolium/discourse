...
export default Controller.extend({
  taken: false, saving: false, errorMessage: null, newUsername: null,
    ...
    @observes("newUsername") checkTaken() {
    ...
    if (newUsername && newUsername.length < this.minLength) {
      ...
      User.checkUsername(newUsername, undefined, this.get("model.id"))
        .then(result => {
          if (result.errors) {
    ...
    actions: {
      changeUsername() {
        ...
        return bootbox.confirm(
          I18n.t("user.change_username.confirm"), I18n.t("no_value"),
          I18n.t("yes_value"), result => {
            if (result) {
              this.set("saving", true);
              this.model.changeUsername(this.newUsername)
                .then(() => {
