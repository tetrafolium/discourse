...
export default Controller.extend(CanCheckEmails, {
  ...
  ,
    ...
    canEditName: setting("enable_names"), canSaveUser: true,

    newNameInput: null, newTitleInput: null, newPrimaryGroupInput: null,
  ...
  ,
    ...
    @discourseComputed() nameInstructions() {
    return I18n.t(this.siteSettings.full_name_required
  ...
  ,
    ...
    return (primaryGroupOptions.length > 0 &&
            this.siteSettings.user_selected_primary_groups);
  }
  ,

    @discourseComputed("model.is_anonymous") canChangePassword(isAnonymous) {
    if (isAnonymous) {
      return false;
    } else {
      return (!this.siteSettings.enable_sso &&
  ...
  ,
    ...
    @discourseComputed("model.associated_accounts.[]") authProviders(accounts) {
    ...
    const result = allMethods.map(method => {
      return {
        ...
        account: accounts.find(account =>
                                 account.name ===
                                 method.name) // Will be undefined if no account
      };
    });
  ...
  ,
    ...
    @discourseComputed("model.second_factor_enabled", "canCheckEmails",
                       "model.is_anonymous")
  ...
  ,
    ...
    return showAllAuthTokens ? tokens
  ...
  ,
    ...
    actions: {
      ...
      revokeAccount(account) {
        ...
        this.model.revokeAssociatedAccount(account.name)
          .then(result => {
            if (result.success) {
      ...
      revokeAuthToken(token) {
        ajax(
          ...
          { type: "POST", data: token ? { token_id: token.id } : {} })
          .then(() => {
