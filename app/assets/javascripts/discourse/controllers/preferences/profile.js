...
export default Controller.extend({
  ...
  ,
    ...
    @discourseComputed("model.user_fields.@each.value") userFields() {
  ...
  ,
    ...
    @discourseComputed("model.can_change_bio") canChangeBio(canChangeBio) {
    return canChangeBio;
  }
  ,
    ...
    actions: {
      showFeaturedTopicModal() {
        showModal(
          ...
          { model: this.model, title: "user.feature_topic_on_profile.title" });
      },

      clearFeaturedTopicFromProfile() {
        bootbox.confirm(
          I18n.t("user.feature_topic_on_profile.clear.warning"), result => {
            if (result) {
      ...
      save() {
        ...
        const model = this.model, userFields = this.userFields;
        ...
        return model.save(this.saveAttrNames)
          .then(() => {
