...
export default Controller.extend(bufferedProperty("model"), {
  composer: controller(), application: controller(), multiSelect: false,
    selectedPostIds: null, editingTopic: false,
    queryParams: ["filter", "username_filters"],
    loadedAllPosts:
      or("model.postStream.loadedAllPosts", "model.postStream.loadingLastPost"),
    enteredAt: null, enteredIndex: null, retrying: false,
    userTriggeredProgress: null, _progressIndex: null, hasScrolled: null,
    username_filters: null, filter: null, quoteState: null,
  ...
  ,
    ...
    @observes("model.title", "category") _titleChanged() {
  ...
  ,
    ...
    return ((canCreatePost || composerOpened) && this.currentUser &&
  ...
  ,
    ...
    @discourseComputed("model") pmPath(topic) {
  ...
  ,
    ...
    this.setProperties({ selectedPostIds: [], quoteState: new QuoteState() });
  }
  ,
  ...
  ,
    ...
    @discourseComputed selectedQuery() {
  ...
  ,
    ...
    const categoryIds =
      this.site.get("topic_featured_link_allowed_category_ids");
    return (categoryIds === undefined || !categoryIds.length ||
            categoryIds.includes(categoryId));
  }
  ,

    @discourseComputed("model") featuredLinkDomain(topic) {
    return extractLinkMeta(topic).domain;
  }
  ,

    @discourseComputed("model.isPrivateMessage") canEditTags(isPrivateMessage) {
  ...
  ,
    ...
    if (posts) {
      ...
      posts.forEach(post => {
        if (post.post_type === small_action ||
  ...
  ,
    ...
    return ajax(url, {
             data: _.merge({ post_number: post.get("post_number") },
  ...
  ,
    ...
    actions: {
      ...
      deferTopic() {
        ...
        screenTrack.stop();
        const goToPath =
          topic.get("isPrivateMessage") ? currentUser.pmPath(topic) : "/";
      ...
      replyToPost(post) {
        ...
        if (composerController.get("model.topic.id") === topic.get("id") &&
      ...
      deletePost(post) {
        ...
        if (user.get("staff") && hasReplies) {
          ajax(`/posts/${post.id}/reply-ids.json`).then(replies => {
            if (replies.length === 0) {
              return post.destroy(user).then(refresh).catch(error => {
            ...
            buttons.push(
              { label: I18n.t("cancel"), class: "btn-danger right" });

            buttons.push({
              label: I18n.t("post.controls.delete_replies.just_the_post"),
              callback() {
                post.destroy(user).then(refresh).catch(error => {
                  popupAjaxError(error);
                  post.undoDeleteState();
                });
              }
            ...
            if (replies.some(r => r.level > 1)) {
              buttons.push({
                ...
                callback() {
                  loadedPosts.forEach(
                    p => (p === post || replies.some(r => r.id === p.id)) &&
            ...
            const directReplyIds =
              replies.filter(r => r.level === 1).map(r => r.id);
            ...
            buttons.push({
              ...
              callback() {
                loadedPosts.forEach(
                  p => (p === post || directReplyIds.includes(p.id)) &&
                       p.setDeletedState(user));
            ...
            bootbox.dialog(I18n.t("post.controls.delete_replies.confirm"),
                           buttons);
          });
        } else {
          return post.destroy(user).then(refresh).catch(error => {
      ...
      editPost(post) {
        ...
        const opts = {
          post,
          action: editingSharedDraft ? Composer.EDIT_SHARED_DRAFT
                                     : Composer.EDIT,
          draftKey: post.get("topic.draft_key"),
          draftSequence: post.get("topic.draft_sequence")
        };
      ...
      toggleBookmark(post) {
        if (!this.currentUser) {
          ...
          return this.model.toggleBookmark().then(changedIds => {
            ...
            changedIds.forEach(
              id => this.appEvents.trigger("post-stream:refresh", { id }));
          });
        }
      },
      ...
      jumpToPostPrompt() {
        const topic = this.model;
        const modal =
          showModal("jump-to-post", { modalClass: "jump-to-post-modal" });
        modal.setProperties({
          topic,
          postNumber: null,
      ...
      jumpEnd() {
        this.appEvents.trigger("topic:jump-to-post",
                               this.get("model.highest_post_number"));
        DiscourseURL.routeTo(this.get("model.lastPostUrl"), { jumpEnd: true });
      },
      ...
      selectAll() {
        ...
        this.set("selectedPostIds",
                 [...this.get("model.postStream.stream")
                    .filter(postId => !smallActionsPostIds.has(postId))]);
        this._forceRefreshPostStream();
      },
      ...
      togglePostSelection(post) {
        ...
        selected.includes(post.id) ? selected.removeObject(post.id)
      ...
      deleteSelected() {
        ...
        bootbox.confirm(
          I18n.t("post.delete.confirm", { count: this.selectedPostsCount }),
          result => {
            if (result) {
              ...
              this.get("model.postStream.posts")
                .forEach(p => this.postSelected(p) && p.setDeletedState(user));
      ...
      removeNotice(post) {
        return post.updatePostField("notice", null)
          .then(() =>
                  post.setProperties({ notice_type: null, notice_args: null }));
      },
      ...
      pinGlobally() {
        const topic = this.model, until = this.get("model.pinnedGloballyUntil");

        // optimistic update
        topic.setProperties(
          { pinned_at: moment(), pinned_globally: true, pinned_until: until });
      ...
      replyAsNewTopic(post, quotedText) {
        ...
        composerController.open(options)
          ...
          .then(q => {
            const postUrl =
              `${location.protocol}//${location.host}${post.get("url")}`;
            const postLink =
              `[${Handlebars.escapeExpression(this.get("model.title"))}](${
                postUrl})`;
      ...
      convertToPublicTopic() {
        showModal("convert-to-public-topic",
                  { model: this.model, modalClass: "convert-to-public-topic" });
      },
      ...
      removeTopicTimer(statusType, topicTimer) {
        TopicTimer
          .updateStatus(this.get("model.id"), null, null, statusType, null)
  ...
  ,
    ...
    postStream.loadNearestPostToDate(date)
      .then(post => {
  ...
  ,
    ...
    @discourseComputed("selectedPostIds", "model.postStream.posts",
                       "selectedPostIds.[]", "model.postStream.posts.[]")
  ...
  ,
    ...
    @discourseComputed("selectedPostsCount", "selectedPosts",
                       "selectedPosts.[]")
    ...
    return selectedPosts.every(p => p.username === username) ? username
                                                             : undefined;
  }
  ,

    @discourseComputed("selectedPostsCount", "model.postStream.isMegaTopic",
                       "model.postStream.stream.length", "model.posts_count")
    selectedAllPosts(selectedPostsCount, isMegaTopic, postsCount,
                     topicPostsCount) {
    if (isMegaTopic) {
  ...
  ,
    ...
    @discourseComputed("currentUser.staff", "selectedPostsCount",
                       "selectedAllPosts", "selectedPosts", "selectedPosts.[]")
    canDeleteSelected(isStaff, selectedPostsCount, selectedAllPosts,
                      selectedPosts) {
    return (selectedPostsCount > 0 && ((selectedAllPosts && isStaff) ||
  ...
  ,
    ...
    @discourseComputed("currentUser.admin", "selectedPostsCount",
                       "selectedPostsUsername")
    canChangeOwner(isAdmin, selectedPostsCount, selectedPostsUsername) {
    return (isAdmin && selectedPostsCount > 0 &&
            selectedPostsUsername !== undefined);
  }
  ,

    @discourseComputed("selectedPostsCount", "selectedPostsUsername",
                       "selectedPosts", "selectedPosts.[]")
    canMergePosts(selectedPostsCount, selectedPostsUsername, selectedPosts) {
    return (selectedPostsCount > 1 && selectedPostsUsername !== undefined &&
            selectedPosts.every(p => p.can_delete));
  }
  ,

    @observes("multiSelect") _multiSelectChanged() {
    this.set("selectedPostIds", []);
  }
  ,
  ...
  ,
    ...
    this.messageBus.subscribe(`/topic/${this.get("model.id")}`, data => {
      ...
      if (isPresent(data.notification_level_change)) {
        ...
        topic.set("details.notifications_reason_id",
      ...
      case "deleted": {
        postStream.triggerDeletedPost(data.id).then(() =>
                                                      refresh({ id: data.id }));
        break;
      }
      case "recovered": {
        postStream.triggerRecoveredPost(data.id).then(
          () => refresh({ id: data.id }));
        break;
      }
      case "created": {
      ...
      if (topic.get("isPrivateMessage") && this.currentUser &&
          this.currentUser.get("id") !== data.user_id && data.user_id === -2 &&
          data.type === "created") {
        ...
        if (notInPostStream && postNumberDifference > 0 &&
  ...
  ,
    ...
    _scrollToPost: discourseDebounce(
      function(postNumber) {
  ...
  ,
    ...
    @observes("model.postStream.loaded",
              "model.postStream.loadedAllPosts") _showFooter() {
    const showFooter = this.get("model.postStream.loaded") &&
