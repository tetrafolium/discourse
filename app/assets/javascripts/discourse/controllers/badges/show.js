...
export default Controller.extend({
  application: controller(), queryParams: ["username"], noMoreBadges: false,
    userBadges: null, hiddenSetTitle: true,

    @discourseComputed("userBadgesAll") filteredList(userBadgesAll) {
    return userBadgesAll.filterBy("badge.allow_title", true);
  }
  ,

    @discourseComputed("filteredList") selectableUserBadges(filteredList) {
    return [
      EmberObject.create(
  ...
  ,
    ...
    @discourseComputed("username") user(username) {
    if (username) {
  ...
  ,
    ...
    @discourseComputed("username", "model.grant_count",
                       "userBadges.grant_count")
  ...
  ,
    ...
    @discourseComputed("model.allow_title", "model.has_badge",
                       "model") canSelectTitle(hasTitleBadges, hasBadge) {
  ...
  ,
    ...
    actions: {
      loadMore() {
        ...
        UserBadge
          .findByBadgeId(this.get("model.id"),
                         { offset: userBadges.length, username: this.username })
          .then(result => {
  ...
  ,
    ...
    @discourseComputed("user", "model.grant_count") canShowOthers(user,
                                                                  grantCount) {
    return !!user && grantCount > 1;
  }
  ,

    @observes("canLoadMore") _showFooter() {
