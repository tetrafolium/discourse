...
export default Controller.extend(
  ModalFunctionality, PasswordValidation, UsernameValidation, NameValidation,
  UserFieldsValidation, {
    login: controller(),

      complete: false, accountChallenge: 0, accountHoneypot: 0,
      formSubmitted: false, rejectedEmails: A(), prefilledUsername: null,
      userFields: null, isDeveloper: false,
    ...
    ,
      ...
      @discourseComputed("passwordRequired", "nameValidation.failed",
                         "emailValidation.failed", "usernameValidation.failed",
                         "passwordValidation.failed",
                         "userFieldsValidation.failed", "formSubmitted",
                         "inviteCode") submitDisabled() {
    ...
    ,
      ...
      @discourseComputed fullnameRequired() {
    ...
    ,
      ...
      @discourseComputed disclaimerHtml() {
      return I18n.t("create_account.disclaimer", {
        tos_link: this.get("siteSettings.tos_url") || Discourse.getURL("/tos"),
        privacy_link: this.get("siteSettings.privacy_policy_url") ||
    ...
    ,
      ...
      if (this.get("authOptions.email") === email &&
    ...
    ,
      ...
      @discourseComputed("accountEmail", "authOptions.email",
                         "authOptions.email_valid") emailValidated() {
      return (this.get("authOptions.email") === this.accountEmail &&
              this.get("authOptions.email_valid"));
    }
    ,
      ...
      return matchingProvider ? matchingProvider.get("prettyName")
                              : providerName;
    }
    ,
      ...
      @observes("emailValidation", "accountEmail") prefillUsername:
        function() {
          ...
          if (this.get("emailValidation.ok") &&
    ...
    ,
      ...
      @on("init") fetchConfirmationValue() {
    ...
    ,
      ...
      const attrs =
        this.getProperties("accountName", "accountEmail", "accountPassword",
                           "accountUsername", "accountChallenge", "inviteCode");
      ...
      return User.createAccount(attrs).then(
        result => {
          ...
          if (result.success) {
            ...
            $hidden_login_form.find("input[name=username]")
              .val(attrs.accountUsername);
            $hidden_login_form.find("input[name=password]")
              .val(attrs.accountPassword);
            $hidden_login_form.find("input[name=redirect]")
              .val(userPath("account-created"));
            $hidden_login_form.submit();
          } else {
            this.flash(result.message || I18n.t("create_account.failed"),
                       "error");
            if (result.is_developer) {
              this.set("isDeveloper", true);
            }
            if (result.errors && result.errors.email &&
                result.errors.email.length > 0 && result.values) {
