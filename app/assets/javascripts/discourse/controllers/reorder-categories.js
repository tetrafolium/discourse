...
const BufferedProxy =
  window
    .BufferedProxy; // import BufferedProxy from 'ember-buffered-proxy/proxy';
...
export default Controller.extend(ModalFunctionality, Evented, {
  ...
  ,
    ...
    @discourseComputed("site.categories") categoriesBuffered(categories) {
   ...
   **/
    ...
    const reorderChildren = (categoryId, depth, index) => {
      this.categoriesOrdered.forEach(category => {
        if ((categoryId === null && !category.get("parent_category_id")) ||
  ...
  ,
    ...
    actions: {
      change(category, event) {
        ...
        newPosition =
          Math.min(Math.max(newPosition, 0), this.categoriesOrdered.length - 1);
