...
export default Controller.extend(CanCheckEmails, {
  ...
  ,
    ...
    @discourseComputed("model.profile_hidden", "indexStream", "viewingSelf",
                       "forceExpand") collapsedInfo(profileHidden, indexStream,
                                                    viewingSelf, forceExpand) {
    if (profileHidden) {
      return true;
    }
  ...
  , canMuteOrIgnoreUser: or("model.can_ignore_user", "model.can_mute_user"),
    ...
    showStaffCounters: or("hasGivenFlags", "hasFlaggedPosts", "hasDeletedPosts",
                          "hasBeenSuspended", "hasReceivedWarnings"),

    showFeaturedTopic:
      and("model.featured_topic",
  ...
  ,
    ...
    @discourseComputed("model.trust_level") removeNoFollow(trustLevel) {
  ...
  ,
    ...
    @discourseComputed("viewingSelf") showDrafts(viewingSelf) {
    return viewingSelf;
  }
  ,
    ...
    return (this.siteSettings.enable_personal_messages &&
            (viewingSelf || isAdmin));
  }
  ,
  ...
  ,
    ...
    @discourseComputed("model.name") nameFirst(name) {
    return prioritizeNameInUx(name, this.siteSettings);
  }
  ,

    @discourseComputed("model.badge_count") showBadges(badgeCount) {
    return Discourse.SiteSettings.enable_badges && badgeCount > 0;
  }
  ,

    @discourseComputed() canInviteToForum() {
  ...
  ,
    ...
    @discourseComputed("model.user_fields.@each.value") publicUserFields() {
    const siteUserFields = this.site.get("user_fields");
    if (!isEmpty(siteUserFields)) {
      const userFields = this.get("model.user_fields");
      return siteUserFields.filterBy("show_on_profile", true)
        .sortBy("position")
        .map(field => {
          set(field, "dasherized_name", field.get("name").dasherize());
          const value =
            userFields ? userFields[field.get("id").toString()] : null;
  ...
  ,
    ...
    userNotificationLevel: computed("currentUser.ignored_ids", "model.ignored",
                                    "model.muted",
                                    function() {
