  ...
  ,
    ...
    actions: {
      ...
      help() {
        this.setProperties({
          offerHelp:
            I18n.t("forgot_password.help", { basePath: Discourse.BaseUri }),
          helpSeen: true
        });
      },
      ...
      resetPassword() {
        ...
        ajax(
          ...
          { data: { login: this.accountEmailOrUsername.trim() }, type: "POST" })
          .then(data => {
            const accountEmailOrUsername =
              escapeExpression(this.accountEmailOrUsername);
            const isEmail = accountEmailOrUsername.match(/@/);
            let key =
              `forgot_password.complete_${isEmail ? "email" : "username"}`;
            let extraClass;
