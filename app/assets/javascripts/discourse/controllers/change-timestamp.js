...
export default Controller.extend(ModalFunctionality, {
  topicController: inject("topic"), saving: false, date: "", time: "",

    @discourseComputed("saving") buttonTitle(saving) {
    return saving ? I18n.t("saving") : I18n.t("topic.change_timestamp.action");
  }
  ,

    @discourseComputed("date", "time") createdAt(date, time) {
    return moment(`${date} ${time}`, "YYYY-MM-DD HH:mm:ss");
  }
  ,

    @discourseComputed("createdAt") validTimestamp(createdAt) {
    ...
    actions: {
      changeTimestamp() {
        ...
        Topic.changeTimestamp(topic.id, this.createdAt.unix())
          ...
          .catch(() => this.flash(I18n.t("topic.change_timestamp.error"),
                                  "alert-error"))
