...
export default Controller.extend(ModalFunctionality, GrantBadgeController, {
  topicController: inject("topic"), loading: true, saving: false,
  ...
  ,
    ...
    @discourseComputed("topicController.selectedPosts") post() {
    return this.get("topicController.selectedPosts")[0];
  }
  ,

    @discourseComputed("post") badgeReason(post) {
  ...
  ,
    ...
    all([
      Badge.findAll(), UserBadge.findByUsername(this.get("post.username"))
    ]).then(([allBadges, userBadges]) => {
      this.setProperties(
        { allBadges: allBadges, userBadges: userBadges, loading: false });
    });
  }
  ,
    ...
    actions: {
      grantBadge() {
        ...
        this
          .grantBadge(this.selectedBadgeId, this.get("post.username"),
                      this.badgeReason)
          .then(
            newBadge => {
