...
export default Controller.extend(ModalFunctionality, {
  loading: true, viewMode: "side_by_side",

    @on("init") _changeViewModeOnMobile() {
  ...
  ,
    ...
    @discourseComputed("previousVersion", "model.current_version",
                       "model.version_count") revisionsText(previous, current,
                                                            total) {
    return I18n.t(
      "post.revisions.controls.comparing_previous_to_current_out_of_total",
      { previous, icon: iconHTML("arrows-alt-h"), current, total });
  }
  ,
  ...
  ,
    ...
    Post.hideRevision(postId, postVersion)
      .then(() => this.refresh(postId, postVersion));
  }
  ,

    show(postId, postVersion) {
    Post.showRevision(postId, postVersion)
      .then(() => this.refresh(postId, postVersion));
  }
  ,

    revert(post, postVersion) {
    post.revertToRevision(postVersion)
      .then(result => {
  ...
  ,
    ...
    @discourseComputed("model.created_at") createdAtDate(createdAt) {
    return moment(createdAt).format("LLLL");
  }
  ,

    @discourseComputed("model.current_version") previousVersion(current) {
    return current - 1;
  }
  ,
  ...
  ,
    ...
    displayGoToFirst:
      propertyGreaterThan("model.current_revision", "model.first_revision"),
    displayGoToNext:
      propertyLessThan("model.current_revision", "model.next_revision"),
    displayGoToLast:
      propertyLessThan("model.current_revision", "model.next_revision"),

    hideGoToFirst: not("displayGoToFirst"),
    hideGoToPrevious: not("displayGoToPrevious"),
    hideGoToNext: not("displayGoToNext"), hideGoToLast: not("displayGoToLast"),
    ...
    @discourseComputed("model.previous_hidden") displayShow(prevHidden) {
    return prevHidden && this.currentUser && this.currentUser.get("staff");
  }
  ,

    @discourseComputed("model.previous_hidden") displayHide(prevHidden) {
    return !prevHidden && this.currentUser && this.currentUser.get("staff");
  }
  ,

    @discourseComputed("model.last_revision", "model.current_revision",
                       "model.can_edit", "topicController")
  ...
  ,
    ...
    @discourseComputed("model.wiki") editButtonLabel(wiki) {
    return `post.revisions.controls.${wiki ? "edit_wiki" : "edit_post"}`;
  }
  ,

    @discourseComputed() displayRevert() {
  ...
  ,
    ...
    @discourseComputed("model.previous_hidden", "model.current_hidden",
                       "displayingInline")
  ...
  ,
    ...
    @discourseComputed("displayingInline") inlineClass(displayingInline) {
  ...
  ,
    ...
    @discourseComputed("model.category_id_changes") previousCategory(changes) {
    if (changes) {
  ...
  ,
    ...
    @discourseComputed("model.category_id_changes") currentCategory(changes) {
    if (changes) {
  ...
  ,
    ...
    @discourseComputed("model.wiki_changes") wikiDisabled(changes) {
    return changes && !changes["current"];
  }
  ,

    @discourseComputed("model.post_type_changes") postTypeDisabled(changes) {
    return (changes && changes["current"] !==
                         this.site.get("post_types.moderator_action"));
  }
  ,

    @discourseComputed("viewMode", "model.title_changes") titleDiff(viewMode) {
  ...
  ,
    ...
    @observes("viewMode", "model.body_changes") bodyDiffChanged() {
    ...
    if (viewMode === "side_by_side_markdown") {
      ...
      return sanitizeAsync(html, opts)
        .then(result => this.set("bodyDiff", result));
    }
  }
  ,
    ...
    actions: {
      loadFirstVersion() {
        this.refresh(this.get("model.post_id"),
                     this.get("model.first_revision"));
      },
      loadPreviousVersion() {
        this.refresh(this.get("model.post_id"),
      ...
      loadNextVersion() {
        this.refresh(this.get("model.post_id"),
                     this.get("model.next_revision"));
      },
      loadLastVersion() {
        this.refresh(this.get("model.post_id"),
                     this.get("model.last_revision"));
      },

      hideVersion() {
        this.hide(this.get("model.post_id"),
                  this.get("model.current_revision"));
      },
      showVersion() {
        this.show(this.get("model.post_id"),
                  this.get("model.current_revision"));
      },
