...
const AuthErrors = [
  "requires_invite", "awaiting_approval", "awaiting_activation",
  "admin_not_allowed_from_ip_address", "not_allowed_from_ip_address"
];

export default Controller.extend(ModalFunctionality, {
  createAccount: controller(), forgotPassword: controller(),
    application: controller(),

    loggingIn: false, loggedIn: false, processingEmailLink: false,
    showLoginButtons: true, showSecondFactor: false, awaitingApproval: false,
  ...
  ,
    ...
    @discourseComputed("loggingIn") loginButtonLabel(loggingIn) {
  ...
  ,
    ...
    actions: {
              ...
              if (result && result.error) {
                ...
                if ((result.security_key_enabled || result.totp_enabled) &&
                  ...
                  this.setProperties({
                    ...
                    securityKeyAllowedCredentialIds:
                      result.allowed_credential_ids
                  });

                  // only need to focus the 2FA input for TOTP
                  if (!this.showSecurityKey) {
                    scheduleOnce("afterRender",
                                 () => document.getElementById("second-factor")
                                         .querySelector("input")
                                         .focus());
                  }
                ...
                const applyHiddenFormInputValue = (value, key) => {
                  ...
                  hiddenLoginForm.querySelector(`input[name=${key}]`).value =
                    value;
                };
                ...
                if (hiddenLoginForm) {
                  if (navigator.userAgent.match(/(iPad|iPhone|iPod)/g) &&
                    ...
                    window.location.href =
                      hiddenLoginForm.querySelector("input[name=redirect]")
                        .value;
                  } else {
      ...
      forgotPassword() {
        ...
        if (forgotPasswordController) {
          forgotPasswordController.set("accountEmailOrUsername",
                                       this.loginName);
        }
      ...
      emailLogin() {
        ...
        ajax("/u/email-login",
             { data: { login: this.loginName.trim() }, type: "POST" })
          .then(data => {
            ...
            if (data.user_found === false) {
              this.flash(I18n.t(`${key}_not_found`,
                                { email: loginName, username: loginName }),
                         "error");
            } else {
              let postfix = data.hide_taken ? "" : "_found";
              this.flash(I18n.t(`${key}${postfix}`,
                                { email: loginName, username: loginName }));
            }
          })
      ...
      authenticateSecurityKey() {
        getWebauthnCredential(
          this.securityKeyChallenge, this.securityKeyAllowedCredentialIds,
          credentialData => {
    ...
    if (options.awaiting_approval && !this.canLoginLocal &&
    ...
    if (options.omniauth_disallow_totp) {
      return loginError(I18n.t("login.omniauth_disallow_totp"), "error", () => {
        this.setProperties(
          { loginName: options.email, showLoginButtons: false });
