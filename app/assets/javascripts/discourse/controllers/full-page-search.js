...
export default Controller.extend({
  application: controller(), composer: controller(), bulkSelectEnabled: null,

    loading: false,
    queryParams: ["q", "expanded", "context_id", "context", "skip_context"],
    q: null, selected: [], expanded: false, context_id: null, context: null,
    searching: false, sortOrder: 0, sortOrders: SortOrders,
    invalidSearch: false, page: 1, resultCount: null,

    @discourseComputed("resultCount") hasResults(resultCount) {
    return (resultCount || 0) > 0;
  }
  ,

    @discourseComputed("q") hasAutofocus(q) {
    return isEmpty(q);
  }
  ,

    @discourseComputed("q") highlightQuery(q) {
    if (!q) {
      return;
    }
  ...
  ,
    ...
    @discourseComputed("skip_context", "context") searchContextEnabled: {
      get(skip, context) {
    ...
    @discourseComputed("context",
                       "context_id") searchContextDescription(context, id) {
    var name = id;
    if (context === "category") {
  ...
  ,
    ...
    @discourseComputed("q") searchActive(q) {
    return isValidSearchTerm(q);
  }
  ,

    @discourseComputed("q") noSortQ(q) {
    q = this.cleanTerm(q);
  ...
  ,
    ...
    @observes("sortOrder") triggerSearch() {
  ...
  ,
    ...
    @discourseComputed("q") showLikeCount(q) {
  ...
  ,
    ...
    @discourseComputed("q") isPrivateMessage(q) {
    return (
      q && this.currentUser &&
      (q.indexOf("in:personal") > -1 ||
       q.indexOf(
         `personal_messages:${this.currentUser.get("username_lower")}`) > -1));
  }
  ,

    @observes("loading") _showFooter() {
    this.set("application.showFooter", !this.loading);
  }
  ,

    @discourseComputed("resultCount", "noSortQ") resultCountLabel(count, term) {
    const plus = count % 50 === 0 ? "+" : "";
    return I18n.t("search.result_count", { count, plus, term });
  }
  ,

    @observes("model.posts.length") resultCountChanged() {
    this.set("resultCount", this.get("model.posts.length"));
  }
  ,

    @discourseComputed("hasResults") canBulkSelect(hasResults) {
  ...
  ,
    ...
    @discourseComputed("page") isLastPage(page) {
  ...
  ,
    ...
    if ((!skip && this.context) || skip === "false") {
      args.search_context = { type: this.context, id: this.context_id };
    }
      ...
      .then(results => {
        ...
        if (args.page > 1) {
          if (model) {
            ...
            this.model.set("grouped_search_result",
  ...
  ,
    ...
    actions: {
      ...
      loadMore() {
        var page = this.page;
        if (this.get("model.grouped_search_result.more_full_page_results") &&
            !this.loading && page < PAGE_LIMIT) {
      ...
      logClick(topicId) {
        if (this.get("model.grouped_search_result.search_log_id") && topicId) {
          ajax("/search/click", {
            ...
            data: {
              search_log_id:
                this.get("model.grouped_search_result.search_log_id"),
