...
export default Controller.extend(ModalFunctionality, {
  post: null, resolve: null, reject: null,

    notice: null, saving: false,

    @discourseComputed("saving", "notice") disabled(saving, notice) {
  ...
  ,
    ...
    actions: {
      setNotice() {
        ...
        post.updatePostField("notice", notice)
          .then(() => cookAsync(notice, { features: { onebox: false } }))
          .then(cookedNotice => {
            post.setProperties(
              { notice_type: "custom", notice_args: cookedNotice.string });
            resolve();
