...
export default Controller.extend({
  application: controller(), counts: null, showing: "members", destroying: null,
    router: service(), currentPath: readOnly("router._router.currentPath"),

    @discourseComputed("showMessages", "model.user_count",
                       "model.request_count", "canManageGroup",
                       "model.allow_membership_requests")
    tabs(showMessages, userCount, requestCount, canManageGroup,
    ...
    if (showMessages) {
      defaultTabs.push(Tab.create({ name: "messages", i18nKey: "messages" }));
    }
  ...
  ,
    ...
    @discourseComputed("model.is_group_user") showMessages(isGroupUser) {
  ...
  ,
    ...
    @discourseComputed("model.displayName",
                       "model.full_name") groupName(displayName, fullName) {
    return (fullName || displayName).capitalize();
  }
  ,

    @discourseComputed("model.name", "model.flair_url", "model.flair_bg_color",
                       "model.flair_color")
  ...
  ,
    ...
    @discourseComputed("model", "model.automatic") canManageGroup(model,
                                                                  automatic) {
  ...
  ,
    ...
    actions: {
      ...
      destroy() {
        ...
        bootbox.confirm(I18n.t("admin.groups.delete_confirm"),
                        I18n.t("no_value"), I18n.t("yes_value"), confirmed => {
                          if (confirmed) {
                            group.destroy()
                              .then(() => {
                              ...
                              .catch(error => {
                                ...
                                console.error(error);
                                bootbox.alert(
                                  I18n.t("admin.groups.delete_failed"));
                              })
