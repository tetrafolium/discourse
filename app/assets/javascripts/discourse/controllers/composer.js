...
import {authorizesOneOrMoreExtensions, uploadIcon} from "discourse/lib/uploads";
...
export default Controller.extend({
  topicController: inject("topic"), router: service(),

    checkedMessages: false, messageCount: null, showEditReason: false,
    editReason: null, scopedCategoryId: null, lastValidatedAt: null,
    isUploading: false, allowUpload: false, uploadIcon: "upload", topic: null,
    linkLookup: null, showPreview: true,
  ...
  ,
    ...
    @discourseComputed("showPreview") toggleText(showPreview) {
    return showPreview ? I18n.t("composer.hide_preview")
                       : I18n.t("composer.show_preview");
  }
  ,

    @observes("showPreview") showPreviewChanged() {
    if (!this.site.mobileView) {
      this.keyValueStore.set(
        { key: "composer.showPreview", value: this.showPreview });
    }
  }
  ,

    @discourseComputed("model.replyingToTopic", "model.creatingPrivateMessage",
                       "model.targetRecipients", "model.composeState")
    ...
    usernames = usernames || "";
    if ((creatingPM && usernames.length === 0) ||
  ...
  ,
    ...
    showToolbar: computed({
      ...
      set(key, val) {
        ...
        keyValueStore.set(
          { key: "toolbar-enabled", value: val ? "true" : "false" });
        return val;
      }
    }),
    ...
    return (
      this.site.can_tag_topics && canEditTitle && !creatingPrivateMessage &&
  ...
  ,
    ...
    @discourseComputed("model.post") replyingToWhisper(repliedToPost) {
    return (repliedToPost &&
            repliedToPost.post_type === this.site.post_types.whisper);
  }
  ,

    isWhispering: or("replyingToWhisper", "model.whisper"),

    @discourseComputed("model.action", "isWhispering") saveIcon(action,
                                                                isWhispering) {
  ...
  ,
    ...
    @discourseComputed("isStaffUser", "model.action") canWhisper(isStaffUser,
                                                                 action) {
    return (this.siteSettings.enable_whispers && isStaffUser &&
  ...
  ,
    ...
    @discourseComputed("model.composeState", "model.creatingTopic",
                       "model.post") popupMenuOptions(composeState) {
  ...
  ,
    ...
    if (isEmpty(usernames) || usernames.split(",").length !== 1 ||
        hasTargetGroups) {
      return false;
    }
  ...
  ,
    ...
    @discourseComputed("model.topic.title") draftTitle(topicTitle) {
    return emojiUnescape(escapeExpression(topicTitle));
  }
  ,

    @discourseComputed allowUpload() {
    return authorizesOneOrMoreExtensions(this.currentUser.staff);
  }
  ,

    @discourseComputed() uploadIcon() {
  ...
  ,
    ...
    actions: {
      ...
      openComposer(options, post, topic) {
        this.open(options).then(() => {
          ...
          const continueDiscussion =
            I18n.t("post.continue_discussion", { postLink: link });

          const reply = this.get("model.reply");
          if (!reply || !reply.includes(continueDiscussion)) {
            this.model.prependText(continueDiscussion, { new_line: true });
          }
        });
      },
      ...
      toggle() {
        ...
        if (isEmpty(this.get("model.reply")) &&
      ...
      importQuote(toolbarEvent) {
        ...
        if (this.get("model.editingPost")) {
          ...
          if (replyToPostNumber) {
            const replyPost =
              postStream.posts.findBy("post_number", replyToPostNumber);
        ...
        if (postId) {
          ...
          return this.store.find("post", postId).then(post => {
            const quote =
              Quote.build(post, post.raw, { raw: true, full: true });
      ...
      hitEsc() {
        if (document.querySelectorAll(".emoji-picker-modal.fadeIn").length ===
            1) {
      ...
      groupsMentioned(groups) {
        if (!this.get("model.creatingPrivateMessage") &&
      ...
      cannotSeeMention(mentions) {
        mentions.forEach(mention => {
          ...
          const body = I18n.t(translation, { username: `@${mention.name}` });
      ...
      if (currentTopic.id !== composer.get("topic.id")) {
        ...
        buttons.push({
          label: I18n.t("composer.reply_here") +
                   ...
                   currentTopic.get("fancyTitle") + "</div>",
        ...
        buttons.push({
          label: I18n.t("composer.reply_original") +
                   "<br/><div class='topic-title overflow-ellipsis'>" +
                   this.get("model.topic.fancyTitle") + "</div>",
    ...
    const promise =
      composer.save({ imageSizes, editReason: this.editReason })
        .then(result => {
          if (result.responseJson.action === "enqueued") {
            this.send("postWasEnqueued", result.responseJson);
            if (result.responseJson.pending_post) {
              let pendingPosts =
                this.get("topicController.model.pending_posts");
              if (pendingPosts) {
          ...
          if (result.responseJson.action === "create_post") {
            this.appEvents.trigger("post:highlight",
    ...
    if (this.router.currentRouteName.split(".")[0] === "topic" &&
   ...
   **/
    ...
    if (opts.ignoreIfChanged && composerModel &&
        composerModel.composeState !== Composer.CLOSED) {
      return;
    }

    this.setProperties(
      { showEditReason: false, editReason: null, scopedCategoryId: null });
    ...
    return new Promise((resolve, reject) => {
      if (composerModel && composerModel.replyDirty) {
        ...
        if (composerModel.composeState === Composer.OPEN &&
            composerModel.draftKey === opts.draftKey && !opts.action) {
          return resolve();
        }

        // If it's the same draft, just open it up again.
        if (composerModel.composeState === Composer.DRAFT &&
  ...
  ,
    ...
    composerModel.setProperties(
      { composeState: Composer.OPEN, isWarning: false });
  ...
  ,
    ...
    if (key) {
      ...
      return Draft.clear(key, this.get("model.draftSequence"))
        .then(() => this.appEvents.trigger("draft:destroyed", key));
    } else {
    ...
    return new Promise(resolve => {
      if (this.get("model.hasMetaData") || this.get("model.replyDirty")) {
        bootbox.dialog(I18n.t(keyPrefix + ".confirm"), [
          {
            label: differentDraft ? I18n.t(keyPrefix + ".no_save_draft")
  ...
  ,
    ...
    @observes("model.reply", "model.title") _shouldSaveDraft() {
  ...
  ,
    ...
    if (this.site.can_tag_topics && category &&
  ...
  ,
    ...
    @discourseComputed("model.action") canEdit(action) {
    return action === "edit" && this.currentUser.can_edit;
  }
  ,

    @discourseComputed("model.composeState") visible(state) {
