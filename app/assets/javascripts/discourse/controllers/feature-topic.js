...
export default Controller.extend(ModalFunctionality, {
  topicController: inject("topic"),
    ...
    loading: true, pinnedInCategoryCount: 0, pinnedGloballyCount: 0,
    bannerCount: 0,
  ...
  ,
    ...
    @discourseComputed("model.category") categoryLink(category) {
  ...
  ,
    ...
    @discourseComputed("categoryLink") pinMessage(categoryLink) {
  ...
  ,
    ...
    const key = count === 0 ? "topic.feature_topic.not_pinned"
  ...
  ,
    ...
    @discourseComputed("pinDisabled") pinInCategoryValidation(pinDisabled) {
    if (pinDisabled) {
      return EmberObject.create(
        { failed: true, reason: I18n.t("topic.feature_topic.pin_validation") });
    }
  }
  ,
  ...
  ,
    ...
    if (count < 4) {
      ...
      this.send("hideModal");
      bootbox.confirm(I18n.t("topic.feature_topic.confirm_" + name, { count }),
                      I18n.t("no_value"), I18n.t("yes_value"),
                      confirmed => confirmed ? this._forwardAction(action)
    ...
    actions: {
      ...
      pinGlobally() {
        if (this.pinGloballyDisabled) {
          ...
          this._confirmBeforePinning(this.pinnedGloballyCount, "pin_globally",
                                     "pinGlobally");
        }
      },
