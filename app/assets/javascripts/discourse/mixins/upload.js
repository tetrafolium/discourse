...
export default Mixin.create({
  ...
  calculateUploadUrl() {
    return (getUrl(this.getWithDefault("uploadUrl", "/uploads")) +
            ".json?client_id=" + (this.messageBus && this.messageBus.clientId) +
  ...
  _initialize: on(
    ...
    function() {
      ...
      const maxFiles =
        this.getWithDefault("maxFiles", this.siteSettings.simultaneous_uploads);
      ...
      $upload.on("fileuploadsubmit", (e, data) => {
        const opts =
          _.merge({ bypassNewUserRestriction: true, user: this.currentUser },
  ...
  _destroy: on("willDestroyElement",
               function() {
                 this.messageBus &&
