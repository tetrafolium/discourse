...
const User = RestModel.extend({
  hasPMs: gt("private_messages_stats.all", 0),
    ...
    redirected_to_top: { reason: null },

    @discourseComputed("can_be_deleted",
                       "post_count") canBeDeleted(canBeDeleted, postCount) {
    const maxPostCount = Discourse.SiteSettings.delete_all_posts_max;
    return canBeDeleted && postCount <= maxPostCount;
  }
  ,

    @discourseComputed() stream() {
    return UserStream.create({ user: this });
  }
  ,

    @discourseComputed() bookmarks() {
    return Bookmark.create({ user: this });
  }
  ,

    @discourseComputed() postsStream() {
    return UserPostsStream.create({ user: this });
  }
  ,

    @discourseComputed() userDraftsStream() {
  ...
  ,
    ...
    @discourseComputed("username_lower") searchContext(username) {
    return { type: "user", id: username, user: this };
  }
  ,

    @discourseComputed("username", "name") displayName(username, name) {
  ...
  ,
    ...
    return ("background-image: url(" + Discourse.getURLWithCDN(bgUrl) + ")")
      .htmlSafe();
  }
  ,
  ...
  ,
    ...
    @discourseComputed() mutedTopicsPath() {
  ...
  ,
    ...
    @discourseComputed() watchingTopicsPath() {
    return defaultHomepage() === "latest"
  ...
  ,
    ...
    @discourseComputed() trackingTopicsPath() {
    return defaultHomepage() === "latest"
  ...
  ,
    ...
    @discourseComputed("username") username_lower(username) {
    return username.toLowerCase();
  }
  ,

    @discourseComputed("trust_level") trustLevel(trustLevel) {
    return Site.currentProp("trustLevels")
      .findBy("id", parseInt(trustLevel, 10));
  }
  ,

    isBasic: equal("trust_level", 0), isLeader: equal("trust_level", 3),
    isElder: equal("trust_level", 4), canManageTopic: or("staff", "isElder"),

    @discourseComputed("previous_visit_at") previousVisitAt(previous_visit_at) {
    return new Date(previous_visit_at);
  }
  ,

    @discourseComputed("suspended_till") suspended(suspendedTill) {
    return suspendedTill && moment(suspendedTill).isAfter();
  }
  ,

    @discourseComputed("suspended_till") suspendedForever: isForever,

    @discourseComputed("silenced_till") silencedForever: isForever,

    @discourseComputed("suspended_till") suspendedTillDate: longDate,

    @discourseComputed("silenced_till") silencedTillDate: longDate,

    changeUsername(new_username) {
    return ajax(userPath(`${this.username_lower}/preferences/username`),
                { type: "PUT", data: { new_username } });
  }
  ,
  ...
  ,
    ...
    save(fields) {
    let userFields = [
      "bio_raw", "website", "location", "name", "title", "locale",
      "custom_fields", "user_fields", "muted_usernames", "ignored_usernames",
      "profile_background_upload_url", "card_background_upload_url",
      "muted_tags", "tracked_tags", "watched_tags", "watching_first_post_tags",
      "date_of_birth", "primary_group_id"
    ];

    const data = this.getProperties(fields ? _.intersection(userFields, fields)
                                           : userFields);
    ...
    ["muted", "watched", "tracked", "watched_first_post"].forEach(s => {
      if (fields === undefined || fields.includes(s + "_category_ids")) {
        let prop = s === "watched_first_post" ? "watchedFirstPostCategories"
                                              : s + "Categories";
    ...
    return ajax(userPath(`${this.username_lower}.json`),
      ...
      .then(result => {
        ...
        const userProps =
          getProperties(this.user_option, "enable_quoting", "enable_defer",
                        "external_links_in_new_tab", "dynamic_favicon");
  ...
  ,
    ...
    return ajax(
      ...
      { dataType: "json", data: { login: this.username }, type: "POST" });
  }
  ,

    loadSecondFactorCodes(password) {
    return ajax("/u/second_factors.json", { data: { password }, type: "POST" });
  }
  ,

    requestSecurityKeyChallenge() {
    return ajax("/u/create_second_factor_security_key.json", { type: "POST" });
  }
  ,

    registerSecurityKey(credential) {
    return ajax("/u/register_second_factor_security_key.json",
                { data: credential, type: "POST" });
  }
  ,

    createSecondFactorTotp() {
    return ajax("/u/create_second_factor_totp.json", { type: "POST" });
  }
  ,
    ...
    return ajax(
      ...
      { data: { second_factor_token: authToken, name }, type: "POST" });
  }
  ,

    disableAllSecondFactors() {
    return ajax("/u/disable_second_factor.json", { type: "PUT" });
  }
  ,

    updateSecondFactor(id, name, disable, targetMethod) {
    return ajax("/u/second_factor.json", {
      data: { second_factor_target: targetMethod, name, disable, id },
      type: "PUT"
    });
  }
  ,
  ...
  ,
    ...
    @discourseComputed("groups.[]") filteredGroups() {
  ...
  ,
    ...
    @discourseComputed("statsExcludingPms.@each.count") statsCountNonPM() {
  ...
  ,
    ...
    @discourseComputed("stats.@each.isPM") statsExcludingPms() {
  ...
  ,
    ...
    return PreloadStore
      .getAndRemove(`user_${user.get("username")}`,
                    () => {
      ...
      .then(json => {
        ...
        if (!isEmpty(json.user.featured_user_badge_ids)) {
          ...
          json.user.featured_user_badges =
            json.user.featured_user_badge_ids.map(id => userBadgesMap[id]);
        }
  ...
  ,
    ...
    return ajax(userPath(`${this.username_lower}/staff-info.json`))
      .then(info => {
  ...
  ,
    ...
    return (this.staff || this.trust_level > 0 ||
  ...
  ,
    ...
    return ajax("/invites",
                { type: "POST", data: { email, group_names, custom_message } });
  }
  ,

    generateInviteLink(email, group_names, topic_id) {
    return ajax("/invites/link",
                { type: "POST", data: { email, group_names, topic_id } });
  }
  ,
  ...
  ,
    ...
    this.set("watchedFirstPostCategories",
    ...
    delete:
      function() {
        if (this.can_delete_account) {
          return ajax(
            userPath(this.username + ".json"),
            { type: "DELETE", data: { context: window.location.pathname } });
        } else {
  ...
  ,
    ...
    ajax(userPath(this.username + ".json"),
         { type: "PUT", data: { dismissed_banner_key: bannerKey } });
  }
  ,
  ...
  ,
    ...
    return ajax(userPath(`${this.username_lower}/summary.json`)).then(json => {
      ...
      json.topics.forEach(t =>
                            (topicMap[t.id] = store.createRecord("topic", t)));
  ...
  ,
    ...
    return group.get("automatic") ? false
                                  : this.admin || group.get("is_group_owner");
  }
  ,

    @discourseComputed("groups.@each.title", "badges.[]") availableTitles() {
    let titles = [];
    ...
    return _.uniq(titles).sort().map(title => {
      return {
  ...
  ,
    ...
    if (newSize) {
      ...
      $.cookie("text_size", `${newSize}|${seq}`, { path: "/", expires: 9999 });
    } else {
...
User.reopenClass(Singleton, {
  ...
  createCurrent() {
    ...
    if (userJson && userJson.primary_group_id) {
      const primaryGroup =
        userJson.groups.find(group => group.id === userJson.primary_group_id);
      if (primaryGroup) {
  ...
  groupStats(stats) {
    const responses = UserActionStat.create(
      { count: 0, action_type: UserAction.TYPES.replies });
    ...
    result.forEach((item, index) => {
      if (item.action_type === UserAction.TYPES.topics ||
