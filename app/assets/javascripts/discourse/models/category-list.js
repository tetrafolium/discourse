...
CategoryList.reopenClass({
  categoriesFrom(store, result) {
    ...
    ["week", "month"].some(period => {
      const filteredCategories =
        result.category_list.categories.filter(c => c[`topics_${period}`] > 0);
    ...
    result.category_list.categories.forEach(c => {
      ...
      if (c.subcategory_ids) {
        c.subcategories =
          c.subcategory_ids.map(scid => list.findBy("id", parseInt(scid, 10)));
      }
      ...
      case "month":
        ...
        if (stat > 0) {
          c.stat = `<span class="value">${stat}</span> / <span class="unit">${
            unit}</span>`;
      ...
      default:
        ...
        c.statTitle =
          I18n.t("categories.topic_sentence", { count: c.topics_all_time });
        c.pickAll = true;
        break;
      }
  ...
  listForParent(store, category) {
    return ajax(`/categories.json?parent_category_id=${category.get("id")}`)
      .then(result => {
  ...
  list(store) {
    ...
    return PreloadStore.getAndRemove("categories_list", getCategories)
      .then(result => {
        return CategoryList.create({
          categories: this.categoriesFrom(store, result),
