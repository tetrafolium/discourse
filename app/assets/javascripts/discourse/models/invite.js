...
const Invite = EmberObject.extend({
  rescind() {
    ajax("/invites", { type: "DELETE", data: { email: this.email } });
...
Invite.reopenClass({
  ...
  findInvitedBy(user, filter, search, offset) {
    ...
    return ajax(userPath(`${user.username_lower}/invited.json`), { data })
      .then(result => {
  ...
  findInvitedCount(user) {
    ...
    return ajax(userPath(`${user.username_lower}/invited_count.json`))
