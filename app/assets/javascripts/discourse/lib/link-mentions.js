  ...
  if (opts && opts.group) {
    if (opts.mentionable) {
      extra = `data-name='${username}' data-mentionable-user-count='${
        opts.mentionable.user_count}' data-max-mentions='${maxGroupMention}'`;
      extraClass = "notify";
    }
    $e.replaceWith(
      `<a href='${Discourse.getURL("/g/") + username}' class='mention-group ${
        extraClass}' ${extra}>@${username}</a>`);
  } else {
    ...
    $e.replaceWith(
      `<a href='${userPath(username.toLowerCase())}' class='mention ${
        extraClass}' ${extra}>@${formatUsername(username)}</a>`);
  }
}
...
export function fetchUnseenMentions(usernames, topic_id) {
  return ajax(userPath("is_local_username"), { data: { usernames, topic_id } })
    .then(r => {
