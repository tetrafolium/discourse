...
export default function($elem) {
  ...
  loadScript("/javascripts/jquery.magnific-popup.min.js").then(function() {
    ...
    $elem.find("a.lightbox").not(spoilers).magnificPopup({
      ...
      ajax: { tError: I18n.t("lightbox.content_load_error") },

      callbacks: {
        open() {
          const wrap = this.wrap, img = this.currItem.img,
      ...
      image: {
        ...
        titleSrc(item) {
          ...
          if (!Discourse.SiteSettings.prevent_anons_from_downloading_files ||
              User.current()) {
            src.push('<a class="image-source-link" href="' + href + '">' +
                     renderIcon("string", "download") +
                     I18n.t("lightbox.download") + "</a>");
          }
