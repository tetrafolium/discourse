...
export function translateResults(results, opts) {
  ...
  results.groups =
    results.groups
      .map(group => {
        ...
        const fullName = Handlebars.Utils.escapeExpression(group.full_name ||
                                                           group.display_name);
        ...
        const flairBgColor =
          Handlebars.Utils.escapeExpression(group.flair_bg_color);
    ...
    results.tags
      .map(function(tag) {
        ...
        return EmberObject.create(
          { id: tagName, url: Discourse.getURL("/tag/" + tagName) });
      })
      .compact();
  ...
  if (groupedSearchResult) {
    [["topic", "posts"], ["user", "users"], ["group", "groups"],
     ["category", "categories"], ["tag", "tags"]]
      .forEach(function(pair) {
        const type = pair[0];
        const name = pair[1];
        if (results[name].length > 0) {
          const componentName = opts.searchContext &&
  ...
  const noResults = !!(!results.topics.length && !results.posts.length &&
...
export function isValidSearchTerm(searchTerm) {
  if (searchTerm) {
    return (searchTerm.trim().length >=
            Discourse.SiteSettings.min_search_term_length);
  } else {
    return false;
  }
}
