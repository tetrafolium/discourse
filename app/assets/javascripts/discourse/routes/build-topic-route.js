  ...
  return new Promise(function(resolve) {
           ...
           if (extras.cached) {
             ...
             if (cachedList && cachedList.get("filter") === filter &&
                 (cachedList.get("topics.length") || 0) >
                   cachedList.get("per_page") &&
             ...
             session.setProperties(
               { topicList: null, topicListScrollPosition: null });
           }
           ...
           return resolve(store.findFiltered(
             "topicList", { filter, params: filterParams || {} }));
         })
    .then(function(list) {
...
export default function(filter, extras) {
  ...
  return DiscourseRoute.extend({
    ...
    beforeModel() {
      this.controllerFor("navigation/default")
        .set("filterType", filter.split("/")[0]);
    },
    ...
    model(data, transition) {
      ...
      return findTopicList(this.store, this.topicTrackingState, filter,
                           findOpts, findExtras);
    },
    ...
    titleToken() {
      ...
      const filterText =
        I18n.t("filters." + filter.replace("/", ".") + ".title");
    ...
    setupController(controller, model) {
      ...
      this.controllerFor("navigation/default")
        .set("canCreateTopic", model.get("can_create_topic"));
    },
