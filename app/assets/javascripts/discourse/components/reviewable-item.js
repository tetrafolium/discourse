...
export default Component.extend({
  adminTools: optionalService(), tagName: "", updating: null, editing: false,
    _updates: null,

    @discourseComputed("reviewable.type") customClass(type) {
    return type.dasherize();
  }
  ,

    @discourseComputed("reviewable.topic", "reviewable.topic_id",
                       "reviewable.removed_topic_id") topicId(topic, topicId,
                                                              removedTopicId) {
    return (topic && topic.id) || topicId || removedTopicId;
  }
  ,

    @discourseComputed("siteSettings.reviewable_claiming",
                       "topicId") claimEnabled(claimMode, topicId) {
    return claimMode !== "disabled" && !!topicId;
  }
  ,

    @discourseComputed("claimEnabled", "siteSettings.reviewable_claiming",
  ...
  ,
    ...
    @discourseComputed("siteSettings.reviewable_claiming",
                       "reviewable.claimed_by") claimHelp(claimMode,
                                                          claimedBy) {
    if (claimedBy) {
      return claimedBy.id === this.currentUser.id
    ...
    return claimMode === "optional" ? I18n.t("review.claim_help.optional")
  ...
  ,
    ...
    @discourseComputed("reviewable.type") reviewableComponent(type) {
    ...
    let template = Ember.TEMPLATES[`${templatePath}`] ||
  ...
  ,
    ...
    let performAction = () => {
      ...
      return ajax(`/review/${reviewable.id}/perform/${action.id}?version=${
                    version}`,
                  { method: "PUT" })
        .then(result => {
  ...
  ,
    ...
    if (adminTools) {
      ...
      return adminTools[adminToolMethod](
        createdBy, { postId, postEdit, before: performAction });
    }
  }
  ,
    ...
    actions: {
      explainReviewable(reviewable) {
        showModal("explain-reviewable",
                  { title: "review.explain.title", model: reviewable });
      },
      ...
      saveEdit() {
        ...
        this.set("updating", true);
        return this.reviewable.update(updates)
