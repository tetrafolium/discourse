...
export default Component.extend({
  ...
  ,
    ...
    @discourseComputed("model.is_group_user") userIsGroupUser(isGroupUser) {
    return !!isGroupUser;
  }
  ,
  ...
  ,
    ...
    removeFromGroup() {
    this.model.removeMember(this.currentUser)
    ...
    actions: {
      joinGroup() {
        if (this.currentUser) {
          ...
          model.addMembers(this.currentUser.get("username"))
            .then(() => {
      ...
      leaveGroup() {
        ...
        if (this.model.public_admission) {
          ...
          return bootbox.confirm(I18n.t("groups.confirm_leave"),
      ...
      showRequestMembershipForm() {
        if (this.currentUser) {
          showModal("request-group-membership-form", { model: this.model });
        } else {
          this._showLoginModal();
        }
      }
