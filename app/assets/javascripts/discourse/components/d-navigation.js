...
export default Component.extend(FilterModeMixin, {
  router: service(), persistedQueryParams: null,

    tagName: "",

    @discourseComputed("category") showCategoryNotifications(category) {
  ...
  ,
    ...
    @discourseComputed("hasDraft") createTopicLabel(hasDraft) {
    return hasDraft ? "topic.open_draft" : "topic.create";
  }
  ,

    @discourseComputed("category.can_edit") showCategoryEdit:
      canEdit => canEdit,
    ...
    if (this.persistedQueryParams && currentRouteQueryParams) {
      ...
      const supportedKeys =
        currentKeys.filter(i => discoveryKeys.indexOf(i) > 0);
    ...
    return NavItem.buildList(
      category, { filterType, noSubcategories, persistedQueryParams: params });
  }
  ,
