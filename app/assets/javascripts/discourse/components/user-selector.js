...
export default TextField.extend({
  autocorrect: false, autocapitalize: false, name: "user-selector",
    canReceiveUpdates: false, single: false, fullWidthWrap: false,
  ...
  ,
    ...
    @on("willDestroyElement") _destroyAutocompleteInstance() {
    $(this.element).autocomplete("destroy");
    this.element.addEventListener("paste", this._paste);
  }
  ,

    @on("didInsertElement") _createAutocompleteInstance(opts) {
    const bool = n => {
      const val = this[n];
      return val === true || val === "true";
    };

    let selected = [], groups = [], currentUser = this.currentUser,
        ...
        single = bool("single"), allowAny = bool("allowAny"),
        disabled = bool("disabled"), allowEmails = bool("allowEmails"),
    ...
    $(this.element).val(this.usernames).autocomplete({
      ...
      onChangeItems(items) {
        ...
        userSelectorComponent.setProperties(
          { usernames: items.join(","), hasGroups });
        selected = items;
  ...
  ,
    ...
    @observes("usernames") _clearInput() {
