...
export default Component.extend(CardContentsBase, CanCheckEmails, CleansUp, {
  elementId: "user-card", classNames: "user-card",
    triggeringLinkClass: "mention",
    classNameBindings:
      [
        "visible:show", "showBadges", "user.card_background::no-bg",
        "isFixed:fixed", "usernameClass"
      ],
    ...
    showFeaturedTopic:
      and("user.featured_topic",
          "siteSettings.allow_featured_topic_on_user_profiles"),

    @discourseComputed("user.staff") staff:
      isStaff => (isStaff ? "staff" : ""),

      @discourseComputed("user.trust_level") newUser: trustLevel =>
        (trustLevel === 0 ? "new-user" : ""),

      @discourseComputed("user.name") nameFirst(name) {
    return prioritizeNameInUx(name, this.siteSettings);
  }
  ,

    @discourseComputed("username") usernameClass:
      username => (username ? `user-card-${username}` : ""),

      @discourseComputed("username",
                         "topicPostCount") togglePostsLabel(username, count) {
    return I18n.t("topic.filter_to", { username, count });
  }
  ,

    @discourseComputed("user.user_fields.@each.value") publicUserFields() {
    const siteUserFields = this.site.get("user_fields");
    if (!isEmpty(siteUserFields)) {
      const userFields = this.get("user.user_fields");
      return siteUserFields.filterBy("show_on_user_card", true)
        .sortBy("position")
        .map(field => {
  ...
  ,
    ...
    @discourseComputed("user.trust_level") removeNoFollow(trustLevel) {
    return trustLevel > 2 && !this.siteSettings.tl3_links_no_follow;
  }
  ,

    @discourseComputed("user.badge_count",
                       "user.featured_user_badges.length") moreBadgesCount:
      (badgeCount, badgeLength) => badgeCount - badgeLength,
  ...
  ,
    ...
    @observes("user.card_background_upload_url") addBackground() {
  ...
  ,
    ...
    this.setProperties({ user: null, topicPostCount: null });
  }
  ,
