...
export default Component.extend(UploadMixin, {
  classNames: ["image-uploader"], loadingLightbox: false,
  ...
  ,
    ...
    @discourseComputed("placeholderUrl") placeholderStyle(url) {
    if (isEmpty(url)) {
      return "".htmlSafe();
    }
  ...
  ,
    ...
    @discourseComputed("imageUrl") imageCDNURL(url) {
    if (isEmpty(url)) {
      return "".htmlSafe();
    }
  ...
  ,
    ...
    @discourseComputed("imageCDNURL") backgroundStyle(url) {
    return `background-image: url(${url})`.htmlSafe();
  }
  ,

    @discourseComputed("imageUrl") imageBaseName(imageUrl) {
      ...
      toggleLightbox() {
        if (this.imageFilename) {
          ...
          ajax(`/uploads/lookup-metadata`,
               { type: "POST", data: { url: this.imageUrl } })
            .then(json => {
              this.setProperties({
