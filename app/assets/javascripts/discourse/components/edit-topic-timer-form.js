...
export default Component.extend({
  selection: readOnly("topicTimer.status_type"),
    ...
    @discourseComputed("topicTimer.updateTime", "publishToCategory",
  ...
  ,
    ...
    @discourseComputed("topic.visible") excludeCategoryId(visible) {
    if (visible) return this.get("topic.category_id");
  }
  ,

    @on("init") @observes("topicTimer", "topicTimer.execute_at",
                          "topicTimer.duration") _setUpdateTime() {
    let time = null;
  ...
  ,
    ...
    @observes("selection") _updateBasedOnLastPost() {
    if (!this.autoClose) {
  ...
  ,
    ...
    schedule("afterRender", () => {
      if (!this.get("topicTimer.status_type")) {
        this.set("topicTimer.status_type",
