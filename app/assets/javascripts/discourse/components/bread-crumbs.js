...
export default Component.extend({
  classNameBindings: ["hidden:hidden", ":category-breadcrumb"], tagName: "ol",

    @discourseComputed("categories") filteredCategories(categories) {
    return categories.filter(
      category => this.siteSettings.allow_uncategorized_topics ||
                  category.id !== this.site.uncategorized_category_id);
  }
  ,

    @discourseComputed("category.ancestors", "filteredCategories",
                       "noSubcategories")
    categoryBreadcrumbs(categoryAncestors, filteredCategories,
                        noSubcategories) {
    ...
    return zipped.map(record => {
      ...
      const options =
        filteredCategories.filter(c => c.get("parentCategory.id") ===
  ...
  ,
    ...
    @discourseComputed("category") parentCategory(category) {
    deprecated(
  ...
  ,
    ...
    parentCategories: filter(
      ...
      function(c) {
        ...
        if (c.id === this.site.get("uncategorized_category_id") &&
  ...
  ,
    ...
    @discourseComputed("category") hidden(category) {
  ...
  ,
    ...
    return this.categories.filter(c =>
                                    c.get("parentCategory") === firstCategory);
  }
});
