...
export default Component.extend({
  tagName: null,
    ...
    // page which is wrong.
    emailOrUsername: null, hasCustomMessage: false, customMessage: null,
    inviteIcon: "envelope", invitingExistingUserToTopic: false,
  ...
  ,
    ...
    @discourseComputed("isAdmin", "emailOrUsername", "invitingToTopic",
                       "isPrivateTopic", "inviteModel.groupNames.[]",
                       "inviteModel.saving",
                       "inviteModel.details.can_invite_to")
    disabled(isAdmin, emailOrUsername, invitingToTopic, isPrivateTopic,
             groupNames, saving, can_invite_to) {
  ...
  ,
    ...
    @discourseComputed("isAdmin", "emailOrUsername", "inviteModel.saving",
                       "isPrivateTopic", "inviteModel.groupNames.[]",
                       "hasCustomMessage")
    disabledCopyLink(isAdmin, emailOrUsername, saving, isPrivateTopic,
                     groupNames, hasCustomMessage) {
  ...
  ,
    ...
    @discourseComputed("inviteModel.saving") buttonTitle(saving) {
  ...
  ,
    ...
    @discourseComputed("inviteModel") invitingToTopic(inviteModel) {
    return inviteModel !== this.currentUser;
  }
  ,

    @discourseComputed("inviteModel",
                       "inviteModel.details.can_invite_via_email")
  ...
  ,
    ...
    return (isAdmin ||
            (groupUsers && groupUsers.some(groupUser => groupUser.owner)));
  }
  ,

    // Show Groups? (add invited user to private group)
    @discourseComputed("isGroupOwnerOrAdmin", "emailOrUsername",
                       "isPrivateTopic", "isPM", "invitingToTopic",
                       "canInviteViaEmail")
    showGroups(isGroupOwnerOrAdmin, emailOrUsername, isPrivateTopic, isPM,
               invitingToTopic, canInviteViaEmail) {
    return (
      isGroupOwnerOrAdmin && canInviteViaEmail && !isPM &&
  ...
  ,
    ...
    @discourseComputed("emailOrUsername") showCustomMessage(emailOrUsername) {
    return this.inviteModel === this.currentUser || emailValid(emailOrUsername);
  }
  ,

    // Instructional text for the modal.
    @discourseComputed("isPM", "invitingToTopic", "emailOrUsername",
                       "isPrivateTopic", "isAdmin", "canInviteViaEmail")
    inviteInstructions(isPM, invitingToTopic, emailOrUsername, isPrivateTopic,
                       isAdmin, canInviteViaEmail) {
  ...
  ,
    ...
    @discourseComputed("isPrivateTopic") showGroupsClass(isPrivateTopic) {
  ...
  ,
    ...
    @discourseComputed("isPM") errorMessage(isPM) {
    return isPM ? I18n.t("topic.invite_private.error")
                : I18n.t("topic.invite_reply.error");
  }
  ,

    @discourseComputed("canInviteViaEmail") placeholderKey(canInviteViaEmail) {
    ...
    actions: {
      createInvite() {
        ...
        const onerror = e => {
          if (e.jqXHR.responseJSON && e.jqXHR.responseJSON.errors) {
            ...
            this.set("errorMessage", this.isPM
        ...
        if (this.hasGroups) {
          return this.inviteModel.createGroupInvite(this.emailOrUsername.trim())
            .then(data => {
              model.setProperties({ saving: false, finished: true });
              this.get("inviteModel.details.allowed_groups")
                .pushObject(EmberObject.create(data.group));
              this.appEvents.trigger("post-stream:refresh");
            })
            .catch(onerror);
        } else {
          return this.inviteModel
            .createInvite(this.emailOrUsername.trim(), groupNames,
                          this.customMessage)
            .then(result => {
              model.setProperties({ saving: false, finished: true });
              if (!this.invitingToTopic && userInvitedController) {
                Invite
                  .findInvitedBy(this.currentUser,
                ...
                this.get("inviteModel.details.allowed_users")
                  .pushObject(EmberObject.create(result.user));
                this.appEvents.trigger("post-stream:refresh");
              } else if (this.invitingToTopic &&
                         emailValid(this.emailOrUsername.trim()) && result &&
                         result.user) {
      ...
      generateInvitelink() {
        ...
        return model
          ...
          .then(result => {
            model.setProperties(
              { saving: false, finished: true, inviteLink: result });
