...
export default Component.extend(PanEvents, {
  composerOpen: null, info: null, isPanning: false,
  ...
  ,
    ...
    if (info.get("topicProgressExpanded")) {
      info.setProperties({ renderTimeline: true, renderAdminMenuButton: true });
    } else {
      ...
      if (renderTimeline) {
        const width = window.innerWidth,
              ...
              headerHeight =
                (headerContainer && headerContainer.offsetHeight) || 0;
      ...
      info.setProperties(
        { renderTimeline, renderAdminMenuButton: !renderTimeline });
    }
  }
  ,
  ...
  ,
    ...
    @observes("info.topicProgressExpanded") _expanded() {
    if (this.get("info.topicProgressExpanded")) {
      $(window).on("click.hide-fullscreen", e => {
        let $target = $(e.target);
        let $parents = $target.parents();
        if (!$target.is(".widget-button") && !$parents.is(".widget-button") &&
            !$parents.is(".dropdown-menu") &&
            !$parents.is("#discourse-modal") &&
            !$target.is("#discourse-modal") && !$parents.is(".modal-footer") &&
  ...
  ,
    ...
    this._shouldPanClose(event) ? (offset += SWIPE_VELOCITY)
  ...
  ,
    ...
    _shouldPanClose(e) {
    return ((e.deltaY > SWIPE_DISTANCE_THRESHOLD &&
  ...
  ,
    ...
    if (!this.site.mobileView) {
      ...
      $("#reply-control")
        .on("div-resized.discourse-topic-navigation", () => this._checkSize());
    }
  ...
  ,
    ...
    this.appEvents.off("topic:current-post-scrolled", this, this._topicScrolled)
