...
export default Component.extend({
  hasEdits: gt("reviewable.post_version", 1),
    ...
    @discourseComputed("reviewable.post_updated_at") historyClass(updatedAt) {
    return historyHeat(this.siteSettings, new Date(updatedAt));
  }
  ,

    @discourseComputed("reviewable.post_updated_at") editedDate(updatedAt) {
      ...
      showEditHistory() {
        ...
        this.store.find("post", postId).then(post => {
          let historyController =
            showModal("history", { model: post, modalClass: "history-modal" });
