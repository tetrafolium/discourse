...
export default Component.extend({
  classNames: ["flag-action-type"],
    ...
    @discourseComputed("flag.name_key") customPlaceholder(nameKey) {
    return I18n.t("flagging.custom_placeholder_" + nameKey);
  }
  ,

    @discourseComputed("flag.name", "flag.name_key", "flag.is_custom_flag",
                       "username") formattedName(name, nameKey, isCustomFlag,
                                                 username) {
    if (isCustomFlag) {
  ...
  ,
    ...
    @discourseComputed("flag", "selectedFlag") selected(flag, selectedFlag) {
  ...
  ,
    ...
    @discourseComputed("message.length") customMessageLength(messageLength) {
