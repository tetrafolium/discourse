    ...
    on("init",
       function() {
         ...
         if (filter) {
           this.set(
             "classNames",
             ["user-stream", "filter-" + filter.toString().replace(",", "-")]);
         }
       }),

    loading: false, eyelineSelector: ".user-stream .item",
    classNames: ["user-stream"],

    @observes("stream.user.id") _scrollTopOnModelChange:
      function() {
    ...
    _inserted:
      on("didInsertElement",
         function() {
           ...
           $(this.element)
             .on("click.details-disabled", "details.disabled", () => false);
           $(this.element)
             .on("click.discourse-redirect", ".excerpt a", function(e) {
