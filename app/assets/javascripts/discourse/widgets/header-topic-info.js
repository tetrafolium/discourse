...
createWidget("topic-header-participant", {
  ...
  html(attrs) {
    ...
    return h("a.icon", {
      attributes: { href: url, "data-auto-route": true, title: attrs.username }
    },
             content);
  },
  ...
  click(e) {
    ...
    this.appEvents.trigger(`topic-header:trigger-${this.attrs.type}-card`,
                           this.attrs.username, $target);
    e.preventDefault();
  }
});
...
export default createWidget("header-topic-info", {
  ...
  html(attrs, state) {
    ...
    if (loaded || category) {
      if (category && (!category.get("isUncategorizedCategory") ||
      ...
      if (showPM) {
        ...
        const totalParticipants = topicDetails.allowed_users.length +
        ...
        topicDetails.allowed_users.some(user => {
          ...
          participants.push(
            this.attach("topic-header-participant",
                        { type: "user", user, username: user.username }));
        });
        ...
        topicDetails.allowed_groups.some(group => {
          ...
          participants.push(
            this.attach("topic-header-participant",
                        { type: "group", group, username: group.name }));
        });
    ...
    return h("div.extra-info",
             { className: title.length > 1 ? "two-rows" : "" }, contents);
  },
