...
export default createWidget("poster-name", {
  ...
  settings: {showNameAndGroup: true, showGlyph: true},
  ...
  html(attrs) {
    ...
    const nameFirst = this.siteSettings.display_name_on_posts &&
                      prioritizeNameInUx(name, this.siteSettings);
    const classNames =
      nameFirst ? ["first", "full-name"] : ["first", "username"];
    ...
    if (name && this.siteSettings.display_name_on_posts &&
