...
export default class PostCooked {
  constructor(attrs, decoratorHelper, currentUser) {
    ...
    this.ignoredUsers =
      this.currentUser ? this.currentUser.ignored_users : null;
  }
    ...
    if ($aside.data("expanded")) {
      ...
      ajax(`/posts/by_number/${topicId}/${postId}`)
        .then(result => {
          ...
          div.highlight(
            originalText,
            { caseSensitive: true, element: "span", className: "highlighted" });
        ...
        .catch(e => {
          if ([403, 404].includes(e.jqXHR.status)) {
            ...
            $blockQuote.showHtml(
              $(`<div class='expanded-quote'>${iconHTML(icon)}</div>`), "fast",
              finished);
          }
        });
    } else {
      ...
      $("blockquote", $aside)
        .showHtml($aside.data("original-contents"), "fast", finished);
    }
    return false;
  }

  _urlForPostNumber(postNumber) {
    return postNumber > 0 ? `${this.attrs.topicUrl}/${postNumber}`
                          : this.attrs.topicUrl;
  }
  ...
  _updateQuoteElements($aside, desc) {
    ...
    if (topicNumber && topicNumber === this.attrs.topicId && postNumber) {
      ...
      navLink = `<a href='${this._urlForPostNumber(postNumber)}' title='${
        quoteTitle}' class='back'>${icon}</a>`;
    }
  ...
  _computeCooked() {
    if ((this.attrs.firstPost || this.attrs.embeddedPost) &&
        this.ignoredUsers && this.ignoredUsers.length > 0 &&
