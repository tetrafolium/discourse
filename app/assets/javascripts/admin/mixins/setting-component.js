...
const CUSTOM_TYPES = [
  "bool", "enum", "list", "url_list", "host_list", "category_list",
  "value_list", "category", "uploaded_image_list", "compact_list",
  "secret_list", "upload", "group_list", "tag_list", "color"
];
...
export default Mixin.create({
  classNameBindings: [":row", ":setting", "overridden", "typeClass"],
    content: alias("setting"), validationMessage: null,
    isSecret: oneWay("setting.secret"),

    @discourseComputed("buffered.value", "setting.value") dirty(bufferVal,
                                                                settingVal) {
  ...
  ,
    ...
    @discourseComputed("setting", "buffered.value") preview(setting, value) {
    ...
    if (preview) {
      return new Handlebars.SafeString(
        "<div class='preview'>" + preview.replace(/\{\{value\}\}/g, value) +
        "</div>");
    }
  }
  ,

    @discourseComputed("componentType") typeClass(componentType) {
    return componentType.replace(/\_/g, "-");
  }
  ,

    @discourseComputed("setting.setting", "setting.label") settingName(setting,
                                                                       label) {
    return label || setting.replace(/\_/g, " ");
  }
  ,

    @discourseComputed("type") componentType(type) {
    return CUSTOM_TYPES.indexOf(type) !== -1 ? type : "string";
  }
  ,

    @discourseComputed("setting") type(setting) {
  ...
  ,
    ...
    @discourseComputed("typeClass") componentName(typeClass) {
    return "site-settings/" + typeClass;
  }
  ,

    @discourseComputed("setting.anyValue") allowAny(anyValue) {
  ...
  ,
    ...
    @discourseComputed("buffered.value") bufferedValues: splitPipes,

    @discourseComputed("setting.defaultValues") defaultValues: splitPipes,
  ...
  ,
    ...
    _watchEnterKey:
      on("didInsertElement",
         function() {
           $(this.element)
             .on("keydown.setting-enter", ".input-setting-string", e => {
               if (e.keyCode === 13) {
                 // enter key
                 this.send("save");
               }
    ...
    _removeBindings: on("willDestroyElement",
                        function() {
          ...
          ajax(`/admin/site_settings/${key}/user_count.json`, {
            ...
            if (count > 0) {
              const controller = showModal("site-setting-default-categories", {
                model:
                  { count: result.user_count, key: key.replace(/_/g, " ") },
                admin: true
              });
