...
const ApiKey = RestModel.extend({
  user: computed("_user", {
    ...
    @discourseComputed("description") shortDescription(description) {
  ...
  ,
    ...
    revoke() {
    return ajax(`${this.basePath}/revoke`, { type: "POST" })
      .then(result => this.setProperties(result.api_key));
  }
  ,

    undoRevoke() {
    return ajax(`${this.basePath}/undo-revoke`, { type: "POST" })
      .then(result => this.setProperties(result.api_key));
  }
  ,
  ...
  ,
    ...
    return this.store.adapterFor("api-key").pathFor(this.store, "api-key",
                                                    this.id);
  }
});
