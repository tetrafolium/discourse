...
const Backup = EmberObject.extend({
  ...
  restore() {
    return ajax(
      "/admin/backups/" + this.filename + "/restore",
      { type: "POST", data: { client_id: window.MessageBus.clientId } });
  }
});
...
Backup.reopenClass({
  find() {
    return ajax("/admin/backups.json")
      ...
      .catch(error => {
        bootbox.alert(I18n.t("admin.backups.backup_storage_error",
  ...
  cancel() {
    return ajax("/admin/backups/cancel.json", { type: "DELETE" })
      .then(result => {
        if (!result.success) {
          bootbox.alert(result.message);
        }
  ...
  rollback() {
    return ajax("/admin/backups/rollback.json", { type: "POST" })
      .then(result => {
        if (!result.success) {
          bootbox.alert(result.message);
        } else {
