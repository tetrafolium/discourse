...
const ColorScheme = EmberObject.extend({
  ...
  ,
    ...
    copy() {
    const newScheme =
      ColorScheme.create({ name: this.name, can_edit: true, colors: A() });
    this.colors.forEach(c => {
  ...
  ,
    ...
    @discourseComputed("name", "colors.@each.changed", "saving") changed(name) {
  ...
  ,
    ...
    @discourseComputed("changed") disableSave(changed) {
    if (this.theme_id) {
      return false;
    }
  ...
  ,
    ...
    return ajax("/admin/color_schemes" + (this.id ? "/" + this.id : "") +
                  ".json",
                {
