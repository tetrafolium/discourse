...
const Theme = RestModel.extend({
  isActive: or("default", "user_selectable"),
    ...
    @discourseComputed("theme_fields.[]") targets() {
    return [
      { id: 0, name: "common" }, { id: 1, name: "desktop", icon: "desktop" },
  ...
  ,
    ...
    @discourseComputed("theme_fields.[]") fieldNames() {
    const common =
  ...
  ,
    ...
    @discourseComputed("fieldNames", "theme_fields.[]",
                       "theme_fields.@each.error") fields(fieldNames) {
    const hash = {};
      ...
      hash[target] = fieldNames[target].map(fieldName => {
        ...
        if (target === "translations" || target === "extra_scss") {
          ...
          field.translatedName =
            I18n.t(`admin.customize.theme.${fieldName}.text`);
  ...
  ,
    ...
    @discourseComputed("theme_fields") themeFields(fields) {
    if (!fields) {
  ...
  ,
    ...
    @discourseComputed("theme_fields", "theme_fields.[]") uploads(fields) {
    if (!fields) {
      return [];
    }
    return fields.filter(f => f.target === "common" &&
                              f.type_id === THEME_UPLOAD_VAR);
  }
  ,

    @discourseComputed("theme_fields",
                       "theme_fields.@each.error") isBroken(fields) {
    return fields && fields.any(field => field.error && field.error.length > 0);
  }
  ,

    @discourseComputed("theme_fields.[]") editedFields(fields) {
    return fields.filter(field => !isBlank(field.value) &&
                                  field.type_id !== SETTINGS_TYPE_ID);
  }
  ,

    @discourseComputed("remote_theme.last_error_text") remoteError(errorText) {
  ...
  ,
    ...
    if (name) {
      ...
      return fields.any(field =>
                          field.target === target && !isEmpty(field.value));
    }
  }
  ,
  ...
  ,
    ...
    if (type_id && type_id > 1) {
      let fields = this.theme_fields;
      let existing = fields.find(f => f.target === target && f.name === name &&
                                      f.type_id === type_id);
      if (existing) {
    ...
    if (!existingField) {
      ...
      const changed = (isEmpty(existingField.value) && !isEmpty(value)) ||
  ...
  ,
    ...
    @discourseComputed("childThemes.[]") child_theme_ids(childThemes) {
    if (childThemes) {
  ...
  ,
    ...
    @discourseComputed("name", "default") description:
      function(name, isDefault) {
        if (isDefault) {
  ...
  ,
    ...
    return ajax(this.diffLocalChangesUrl).then(json => {
      if (json && json.error) {
        bootbox.alert(
          I18n.t("generic_error_with_reason", { error: json.error }));
      } else if (json && json.diff) {
        bootbox.confirm(I18n.t("admin.customize.theme.update_confirm") +
                          `<pre><code class="diff">${
                            escapeExpression(json.diff)}</code></pre>`,
                        I18n.t("cancel"),
