...
export default Component.extend({
  editorId: reads("fieldName"),
    ...
    @discourseComputed("fieldName") currentEditorMode(fieldName) {
    return fieldName === "css" ? "scss" : fieldName;
  }
  ,

    @discourseComputed("fieldName", "styles.html",
                       "styles.css") resetDisabled(fieldName) {
    return (this.get(`styles.${fieldName}`) ===
            this.get(`styles.default_${fieldName}`));
  }
  ,

    @discourseComputed("styles", "fieldName") editorContents: {
    ...
    actions: {
      reset() {
        bootbox.confirm(I18n.t("admin.customize.email_style.reset_confirm", {
