...
export default Component.extend({
  classNames: ["admin-backups-logs"], showLoadingSpinner: false,
    hasFormattedLogs: false, noLogsMessage: I18n.t("admin.backups.logs.none"),
  ...
  ,
    ...
    @on("init") @observes("logs.[]") _resetFormattedLogs() {
  ...
  ,
    ...
    @on("init") @observes("logs.[]") _updateFormattedLogs: discourseDebounce(
      function() {
        ...
        this.setProperties(
          { formattedLogs: formattedLogs, index: logs.length });
        // force rerender
        this.renderLogs();
