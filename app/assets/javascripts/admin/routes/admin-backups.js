...
export default DiscourseRoute.extend({
  activate() {
    this.messageBus.subscribe(LOG_CHANNEL, log => {
      if (log.message === "[STARTED]") {
        ...
        this.controllerFor("adminBackups")
  ...
  model() {
    return PreloadStore
      ...
      .then(status => BackupStatus.create({
  ...
  actions: {
    ...
    destroyBackup(backup) {
      bootbox.confirm(I18n.t("admin.backups.operations.destroy.confirm"),
    ...
    startRestore(backup) {
      bootbox.confirm(I18n.t("admin.backups.operations.restore.confirm"),
                      I18n.t("no_value"), I18n.t("yes_value"), confirmed => {
                        if (confirmed) {
                          this.transitionTo("admin.backups.logs");
                          backup.restore();
    ...
    cancelOperation() {
      bootbox.confirm(I18n.t("admin.backups.operations.cancel.confirm"),
                      I18n.t("no_value"), I18n.t("yes_value"), confirmed => {
                        if (confirmed) {
                          Backup.cancel().then(() => {
                            this.controllerFor("adminBackups")
    ...
    rollback() {
      bootbox.confirm(I18n.t("admin.backups.operations.rollback.confirm"),
                      I18n.t("no_value"), I18n.t("yes_value"), confirmed => {
                        if (confirmed) {
                          Backup.rollback();
                        }
    ...
    remoteUploadSuccess() {
      Backup.find().then(backups => {
        this.controllerFor("adminBackupsIndex")
          .set("model", backups.map(backup => Backup.create(backup)));
      });
    }
  }
});
