...
export default Controller.extend({
  sample: alias("model.sample"), errors: alias("model.errors"),
    count: alias("model.grant_count"),

    @discourseComputed("count", "sample.length") countWarning(count,
                                                              sampleLength) {
    if (count <= 10) {
  ...
  ,
    ...
    @discourseComputed("model.query_plan") hasQueryPlan(queryPlan) {
    return !!queryPlan;
  }
  ,

    @discourseComputed("model.query_plan") queryPlanHtml(queryPlan) {
