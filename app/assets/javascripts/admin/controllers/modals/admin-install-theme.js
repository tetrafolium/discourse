...
export default Controller.extend(ModalFunctionality, {
  adminCustomizeThemes: controller(),
    ...
    popular: equal("selection", "popular"), local: equal("selection", "local"),
    remote: equal("selection", "remote"), create: equal("selection", "create"),
    selection: "popular", loading: false,
    keyGenUrl: "/admin/themes/generate_key_pair",
    importUrl: "/admin/themes/import", recordType: "theme",
    checkPrivate: match("uploadUrl", /^git/), localFile: null, uploadUrl: null,
    urlPlaceholder: "https://github.com/discourse/sample_theme",
    advancedVisible: false, selectedType: alias("themesController.currentTab"),
  ...
  ,
    ...
    @discourseComputed("loading", "remote", "uploadUrl", "local", "localFile",
                       "create", "nameTooShort")
    installDisabled(isLoading, isRemote, uploadUrl, isLocal, localFile,
  ...
  ,
    ...
    @observes("privateChecked") privateWasChecked() {
    this.privateChecked
    ...
    if (checked && !this._keyLoading) {
      ...
      ajax(this.keyGenUrl, { method: "POST" })
        .then(pair => {
          this.setProperties(
            { privateKey: pair.private_key, publicKey: pair.public_key });
        })
  ...
  ,
    ...
    @discourseComputed("name") nameTooShort(name) {
    return !name || name.length < MIN_NAME_LENGTH;
  }
  ,

    @discourseComputed("component") placeholder(component) {
    if (component) {
  ...
  ,
    ...
    @discourseComputed("selection") submitLabel(selection) {
    ...
    actions: {
      ...
      installTheme() {
        ...
        if (this.remote || this.popular) {
          options.data = { remote: this.uploadUrl, branch: this.branch };
        ...
        ajax(this.importUrl, options)
          .then(result => {
            const theme =
              this.store.createRecord(this.recordType, result.theme);
