...
export default Controller.extend(CanCheckEmails, {
  adminTools: service(), originalPrimaryGroupId: null,
    customGroupIdsBuffer: null, availableGroups: null, userTitleValue: null,
    ...
    @discourseComputed("model.customGroups") customGroupIds(customGroups) {
    return customGroups.mapBy("id");
  }
  ,

    @discourseComputed("customGroupIdsBuffer",
                       "customGroupIds") customGroupsDirty(buffer, original) {
  ...
  ,
    ...
    @discourseComputed("model.user_fields.[]") userFields(userFields) {
  ...
  ,
    ...
    @discourseComputed("model.can_delete_all_posts", "model.staff",
                       "model.post_count")
  ...
  ,
    ...
    @discourseComputed("model.canBeDeleted",
                       "model.staff") deleteExplanation(canBeDeleted, staff) {
    if (canBeDeleted) {
      return null;
    }
  ...
  ,
    ...
    groupAdded(added) {
    this.model.groupAdded(added).catch(
      () => bootbox.alert(I18n.t("generic_error")));
  }
  ,

    groupRemoved(groupId) {
    this.model.groupRemoved(groupId)
      .then(() => {
  ...
  ,
    ...
    actions: {
      ...
      viewActionLogs() {
        this.adminTools.showActionLogs(
          this, { target_user: this.get("model.username") });
      },
      showSuspendModal() {
      ...
      saveCustomGroups() {
        ...
        bufferedIds.filter(id => !currentIds.includes(id))
          .forEach(id => this.groupAdded(availableGroups.findBy("id", id)));

        currentIds.filter(id => !bufferedIds.includes(id))
      ...
      savePrimaryGroup() {
        ...
        return ajax(path,
                    { type: "PUT", data: { primary_group_id: primaryGroupId } })
