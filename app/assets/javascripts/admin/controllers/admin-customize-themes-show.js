...
import {empty, filterBy, match, mapBy, notEmpty} from "@ember/object/computed";
...
export default Controller.extend({
  downloadUrl: url("model.id", "/admin/customize/themes/%@/export"),
    ...
    @discourseComputed("model.editedFields") editedFieldsFormatted() {
    const descriptions = [];
    ["common", "desktop", "mobile"].forEach(target => {
      ...
      const formattedFields =
        fields.map(f => I18n.t("admin.customize.theme." + f.name + ".text"))
          .join(" , ");
  ...
  ,
    ...
    @discourseComputed("availableChildThemes", "model.childThemes.[]",
                       "model") selectableChildThemes(available, childThemes) {
    if (available) {
      const themes =
        !childThemes
          ? available
  ...
  ,
    ...
    @discourseComputed("allThemes", "model.component",
                       "model") availableChildThemes(allThemes) {
    if (!this.get("model.component")) {
      const themeId = this.get("model.id");
      return allThemes.filter(theme => theme.get("id") !== themeId &&
                                       theme.get("component"));
    }
  }
  ,

    @discourseComputed("model.component") convertKey(component) {
    const type = component ? "component" : "theme";
    return `admin.customize.theme.convert_${type}`;
  }
  ,

    @discourseComputed("model.component") convertIcon(component) {
    return component ? "cube" : "";
  }
  ,

    @discourseComputed("model.component") convertTooltip(component) {
    const type = component ? "component" : "theme";
    return `admin.customize.theme.convert_${type}_tooltip`;
  }
  ,

    @discourseComputed("model.settings") settings(settings) {
  ...
  ,
    ...
    @discourseComputed("model.translations") translations(translations) {
  ...
  ,
    ...
    return this.get("model.editedFields")
      ...
      .then(() => {
        ...
        this.get("parentController.model.content").forEach(theme => {
          ...
          if (index > -1) {
            ...
            theme.setProperties(
              { childThemes: children, child_themes: rawChildren });
          }
        });
      })
  ...
  , transitionToEditRoute() {
    this.transitionToRoute(this.editRouteName, this.get("model.id"), "common",
                           "scss");
  }
  , sourceIsHttp: match("model.remote_theme.remote_url", /^http(s)?:\/\//),

    @discourseComputed("model.remote_theme.remote_url",
                       "model.remote_theme.branch")
    remoteThemeLink(remoteThemeUrl, remoteThemeBranch) {
    return remoteThemeBranch ? `${remoteThemeUrl.replace(/\.git$/, "")}/tree/${
                                 remoteThemeBranch}`
                             : remoteThemeUrl;
  }
  ,
    ...
    actions: {
      updateToLatest() {
        ...
        this.model.updateToLatest().catch(popupAjaxError).finally(() => {
      ...
      checkForThemeUpdates() {
        this.set("updatingRemote", true);
        this.model.checkForUpdates().catch(popupAjaxError).finally(() => {
          this.set("updatingRemote", false);
        });
      },
      ...
      addUpload(info) {
        ...
        model.setField("common", info.name, "", info.upload_id,
                       THEME_UPLOAD_VAR);
      ...
      changeScheme() {
        ...
        this.set("model.color_scheme_id",
      ...
      editTheme() {
        if (this.get("model.remote_theme.is_git")) {
          bootbox.confirm(I18n.t("admin.customize.theme.edit_confirm"),
                          result => {
                            if (result) {
      ...
      removeUpload(upload) {
        return bootbox.confirm(
          ...
          I18n.t("no_value"), I18n.t("yes_value"), result => {
            if (result) {
      ...
      removeChildTheme(theme) {
        this.model.removeChildTheme(theme).then(() =>
      ...
      switchType() {
        ...
        if (relatives && relatives.length > 0) {
          ...
          bootbox.confirm(
            I18n.t(`${this.convertKey}_alert`, { relatives: names.join(", ") }),
            I18n.t("no_value"), I18n.t("yes_value"), result => {
              if (result) {
      ...
      enableComponent() {
        ...
        this.model.saveChanges("enabled").catch(
