...
export default Controller.extend(PeriodComputationMixin, {
  ...
  ,
    ...
    @discourseComputed activityMetricsFilters() {
    return { startDate: this.lastMonth, endDate: this.today };
  }
  ,

    @discourseComputed topReferredTopicsOptions() {
    return { table: { total: false, limit: 8 } };
  }
  ,

    @discourseComputed topReferredTopicsFilters() {
    return {
  ...
  ,
    ...
    @discourseComputed trendingSearchFilters() {
    return {
  ...
  ,
    ...
    @discourseComputed trendingSearchOptions() {
    return { table: { total: false, limit: 8 } };
  }
  ,

    @discourseComputed trendingSearchDisabledLabel() {
      ...
      AdminDashboard.fetchGeneral()
        .then(adminDashboardModel => {
          this.setProperties({
            ...
            reports:
              makeArray(adminDashboardModel.reports).map(x => Report.create(x))
          });
        })
        .catch(e => {
  ...
  ,
    ...
    @discourseComputed("startDate", "endDate") filters(startDate, endDate) {
