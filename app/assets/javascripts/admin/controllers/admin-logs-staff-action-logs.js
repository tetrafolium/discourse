...
export default Controller.extend({
  model: null, filters: null, filtersExists: gt("filterCount", 0),
    userHistoryActions: null,

    @discourseComputed("filters.action_name") actionFilter(name) {
    return name ? I18n.t("admin.logs.staff_actions.actions." + name) : null;
  }
  ,

    @on("init") resetFilters() {
    this.setProperties({
  ...
  ,
    ...
    this.store.findAll("staff-action-log", params).then(result => {
      ...
      if (!this.userHistoryActions) {
        this.set("userHistoryActions",
                 result.extras.user_history_actions
                   .map(action => ({
                          id: action.id,
                          action_id: action.action_id,
                          name: I18n.t("admin.logs.staff_actions.actions." +
                                       action.id),
                          name_raw: action.id
                        }))
      ...
      filterActionIdChanged(filterActionId) {
        if (filterActionId) {
          this._changeFilters({
            ...
            action_id:
              this.userHistoryActions.findBy("id", filterActionId).action_id
          });
        }
      },
