...
export default Controller.extend({
  adminBackups: controller(), status: alias("adminBackups.model"),
    ...
    actions: {
      toggleReadOnlyMode() {
        if (!this.site.get("isReadOnly")) {
          bootbox.confirm(I18n.t("admin.backups.read_only.enable.confirm"),
      ...
      download(backup) {
        ...
        ajax(`/admin/backups/${link}`, { type: "PUT" })
          .then(() => bootbox.alert(
                  I18n.t("admin.backups.operations.download.alert")));
      }
    },

    _toggleReadOnlyMode(enable) {
    ajax("/admin/backups/readonly", { type: "PUT", data: { enable } })
