...
this.workbox.strategies = (function(
  exports, logger_mjs, assert_mjs, cacheNames_mjs, cacheWrapper_mjs,
  fetchWrapper_mjs, getFriendlyURL_mjs, WorkboxError_mjs) {
  'use strict';
  ...
  */
  ...
  class CacheFirst {
    ...
    constructor(options = {}) {
      this._cacheName =
        cacheNames_mjs.cacheNames.getRuntimeName(options.cacheName);
    ...
    async handle({ event, request }) {
      return this.makeRequest({ event, request: request || event.request });
    }
    /**
        ...
        try {
        ...
        } catch (error) {
          {
            logger_mjs.logger.warn(
              ...
              `updating cache for '${
                getFriendlyURL_mjs.getFriendlyURL(request.url)}'.`);
          }
        }
      }
  ...
  */
  ...
  class NetworkFirst {
    ...
    async makeRequest({ event, request }) {
      ...
      if (this._networkTimeoutSeconds) {
        const { id, promise } =
          this._getTimeoutPromise({ request, event, logs });
      ...
      promises.push(
        networkPromise); // Promise.race() will resolve as soon as the first promise resolves.

      let response = await Promise.race(
        promises); // If Promise.race() resolved with null, it might be due to a network
    ...
    _getTimeoutPromise({ request, logs, event }) {
      ...
      const timeoutPromise = new Promise(resolve => {
        ...
        timeoutId =
          setTimeout(onNetworkTimeout, this._networkTimeoutSeconds * 1000);
      });
      return { promise: timeoutPromise, id: timeoutId };
    }
    /**
     * @param {Object} options
